...
=item spamc [-c] [-d host] [-f] [-h] [-S] [-p port] [-s max_size] [-t tout] [-u username] [-e command [args]]
...
Just output the SpamAssassin report text to stdout, if the message is spam.
If the message is ham, nothing will be printed.  The first line of the output
is the number of hits and the threshold, in this format:
