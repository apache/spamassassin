...
=item spamc [-c] [-d addr] [-f] [-h] [-o] [-p port] [-s max_size] [-u username]
...
If using a TCP connection, then specify the hostname to connect to (default B<localhost>).
If using a UNIX socket (through B<-o> option) then specify the socket's path (default F<./socket>).
...
Use a UNIX socket to connect to spamd.  This might not be supported on, or work on, all platforms.
