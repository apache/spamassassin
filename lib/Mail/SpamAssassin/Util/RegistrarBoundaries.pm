...
#
$VALID_TLDS_RE = qr/
(?:X(?:N--(?:MGB(?:A(?:(?:3A4F16|YH7GP)A|AM7A8H|B2BD)|ERP4A5D4AR|C0A9AZCG|BH1A71E|X4CD0AB|9AWBF)|F(?:IQ(?:(?:228C5H|S8|Z9)S|64B)|PCRJ9C3D|ZC2C9E2C|LW351E)|C(?:ZR(?:694B|S0T|U2D)|LCHC0EA0B2G2A9GCD|G4BKI|1AVG)|V(?:ERMGENSBERAT(?:UNG-PW|ER-CT)B|HQUV)|X(?:KC2(?:DL3A5EE0H|AL3HYE2A)|HQ521B)|(?:Q(?:CKA1PM|9JYB4)|GECRJ9|90A3A)C|N(?:QV7F(?:S00EMA)?|GBC5AZD|ODE)|80A(?:S(?:EHDB|WG)|DXHKS|O21A)|3(?:E0B707E|BST00M|DS443G)|4(?:5(?:BRJ9|Q11)C|GBRIM)|KP(?:R(?:W13|Y57)D|UT3I)|P(?:1A(?:CF|I)|GBS0DH)|Y(?:FRO4I67O|GBI2AMMX)|6(?:QQ986B3XL|FRZ82G)|I(?:1B6B1A6A2E|O0A7I)|L(?:GBBAT1AD8J|1ACC)|H(?:2BRJ9C|XT814E)|O(?:GBPF8FL|3CW4H)|S(?:9BRJ9C|ES554G)|J(?:6W193G|1AMH)|55Q(?:W42G|X5D)|D1A(?:CJ3B|LF)|WGB(?:H1C|L6A)|1QQW23A|RHQV96G|ZFR164B|UNUP4Y)|XX|YZ)|C(?:[CDFGKMNVWXZ]|O(?:N(?:S(?:TRUCTION|ULTING)|(?:TRACTOR|DO)S)|M(?:P(?:UTER|ANY)|MUNITY)?|(?:L(?:LEG|OGN)|FFE)E|O(?:[LP]|KING)|UNTRY|ACH|DES)?|A(?:[BL]|R(?:E(?:ERS?)?|AVAN|TIER|DS)|P(?:ETOWN|ITAL)|NCERRESEARCH|T(?:ERING)?|M(?:ERA|P)|S[AH])?|L(?:(?:EAN|OTH)ING|I(?:NIC|CK)|AIMS|UB)?|H(?:R(?:ISTMAS|OME)|ANNEL|URCH|EAP)?|R(?:EDIT(?:CARD)?|(?:UISE)?S|ICKET)?|I(?:T(?:IC|Y))?|E(?:NTER|RN|O)|U(?:ISINELLA)?|Y(?:MRU)?)|S(?:[BDGJLMNRTVXZ]|U(?:PP(?:L(?:IES|Y)|ORT)|R(?:GERY|F)|ZUKI)?|C(?:[AB]|H(?:MIDT|WARZ|ULE)|IENCE|OT)?|O(?:L(?:UTIONS|AR)|FTWARE|CIAL|HU|Y)?|A(?:ARLAND|MSUNG|LE|RL)?|H(?:IKSHA|RIRAM|OES)?|E(?:RVICES|XY|W)?|Y(?:STEMS|DNEY)?|P(?:IEGEL|ACE)|I(?:NGLES)?|KY?)|A(?:[FWZ]|C(?:T(?:IVE|OR)|COUNTANTS|ADEMY)?|U(?:CTION|DIO|TOS)?|L(?:LFINANZ|SACE)?|S(?:SOCIATES|IA)?|R(?:CHI|MY|PA)?|(?:MSTERDA)?M|Q(?:UARELLE)?|I(?:RFORCE)?|T(?:TORNEY)?|(?:BOGAD)?O|N(?:DROID)?|G(?:ENCY)?|D(?:ULT)?|E(?:RO)?|XA?)|B(?:[BDFGHJSTVWY]|U(?:ILD(?:ERS)?|DAPEST|SINESS|ZZ)|L(?:ACK(?:FRIDAY)?|OOMBERG|UE)|A(?:R(?:GAINS)?|N[DK]|YERN)?|E(?:RLIN|ER|ST)?|I(?:[DOZ]|KE)?|N(?:PPARIBAS)?|O(?:UTIQUE|O)?|R(?:USSELS)?|MW?|ZH?)|P(?:[EFGKMNSTWY]|R(?:O(?:D(?:UCTIONS)?|PERT(?:IES|Y)|F)?|AXI|ESS)?|H(?:OTO(?:GRAPHY|S)?|ARMACY|YSIO)?|A(?:R(?:T(?:(?:NER)?S|Y)|IS))?|I(?:C(?:TURE)?S|ZZA|NK)|O(?:KER|HL|RN|ST)|L(?:UMBING|ACE)?|UB)|M(?:[CDGHKLMNPQRSTVWXYZ]|O(?:[EV]|R(?:TGAGE|MON)|N(?:ASH|EY)|TORCYCLES|SCOW|BI|DA)?|A(?:N(?:AGEMENT|GO)|RKET(?:ING)?|DRID|ISON)?|E(?:M(?:ORIAL|E)|LBOURNE|DIA|ET|NU)?|I(?:(?:AM|N)I|L)|U(?:SEUM)?)|G(?:[DFHNPQSTWY]|R(?:A(?:PHIC|TI)S|EEN|IPE)?|L(?:OB(?:AL|O)|ASS|E)?|A(?:L(?:LERY)?|RDEN)?|U(?:I(?:TARS|DE)|RU)?|I(?:FTS?|VES)?|M(?:[OX]|AIL)?|O(?:[PV]|OGLE)|B(?:IZ)?|E(?:NT)?|G(?:EE)?)|F(?:[JM]|I(?:NANC(?:IAL|E)|SH(?:ING)?|T(?:NESS)?|RMDALE)?|L(?:O(?:RIST|WERS)|SMIDTH|IGHTS|Y)|O(?:UNDATION|RSALE|O)?|U(?:RNITURE|TBOL|ND)|A(?:SHION|IL|RM)|R(?:OGANS|L)?|(?:EEDBAC)?K)|E(?:[CEGR]|N(?:GINEER(?:ING)?|TERPRISES|ERGY)|X(?:P(?:OSED|ERT)|CHANGE)|U(?:ROVISION|S)?|(?:QUIPMEN|A)?T|VE(?:RBANK|NTS)|DU(?:CATION)?|M(?:ERCK|AIL)|S(?:TATE|Q)?)|D(?:[JKMZ]|E(?:NT(?:IST|AL)|LIVERY|MOCRAT|GREE|ALS|SI|V)?|I(?:(?:SCOUN|E)T|RECT(?:ORY)?|AMONDS|GITAL)|O(?:(?:MAIN|C)S|OSAN)?|A(?:[DY]|TING|NCE)|URBAN|VAG|NP)|R(?:E(?:P(?:UBLICAN|AIR|ORT)|(?:CIPE|VIEW)S|S(?:TAURAN)?T|I(?:SEN?|T)|N(?:TALS)?|ALTOR|HAB|D)?|O(?:CKS|DEO)?|I(?:[OP]|CH)|S(?:VP)?|U(?:HR)?|YUKYU|W)|T(?:[CDFGHJKLMNPTVWZ]|O(?:(?:OL|Y)S|DAY|KYO|WN|P)?|R(?:A(?:INING|VEL|DE)|UST)?|A(?:T(?:TOO|AR)|IPEI|X)|I(?:R(?:ES|OL)|ENDA|PS)|E(?:CHNOLOGY|L)|UI)|L(?:[BCKRVY]|I(?:M(?:ITED|O)|GHTING|DL|FE|NK)?|A(?:T(?:ROBE)?|CAIXA|WYER|ND)?|O(?:NDON|ANS|TTO)|U(?:X(?:URY|E))?|E(?:ASE|GAL)|T(?:DA)?|D?S|GBT)|V(?:[CGU]|E(?:(?:NTURE|GA)S|RSICHERUNG|T)?|I(?:(?:AJE|LLA)S|SION|DEO)?|O(?:T(?:[EO]|ING)|YAGE|DKA)|(?:LAANDERE)?N|A(?:CATIONS)?)|I(?:[DELOQST]|N(?:[GK]|(?:VESTMENT|DUSTRIE)S|T(?:ERNATIONAL)?|S(?:TITUT|UR)E|FO)?|M(?:MO(?:BILIEN)?)?|R(?:ISH)?|BM|WC)|H(?:[KMNRTU]|O(?:L(?:DINGS|IDAY)|ST(?:ING)?|[RU]SE|MES|W)|E(?:(?:ALTHCA)?RE|LP)|A(?:MBURG|US)|I(?:PHOP|V))|W(?:[FS]|E(?:B(?:SITE|CAM)|D(?:DING)?)|I(?:LLIAMHILL|EN|KI)|A(?:LES|TCH|NG)|OR(?:KS?|LD)|HOSWHO|T[CF]|ME)|N(?:[FLOPUZ]|E(?:T(?:WORK)?|USTAR|XUS|W)?|A(?:GOYA|ME|VY)?|I(?:NJA)?|R[AW]?|GO?|Y?C|HK)|K(?:[EGHMPWYZ]|I(?:TCHEN|WI|M)?|(?:AUFE|OEL)?N|R(?:E?D)?|DDI)|O(?:(?:(?:TSU|SA)K|KINAW)A|RG(?:ANIC)?|N[GL]|OO|VH|M)|Y(?:[ET]|O(?:(?:KOHAM|G)A|UTUBE)|A(?:CHTS|NDEX))|J(?:[MP]|O(?:B(?:URG|S))?|E(?:TZT)?|UEGOS)|U(?:[AGKSYZ]|N(?:IVERSITY|O)|OL)|Z(?:[AMW]|UERICH|ONE|IP)|Q(?:UEBEC|PON|A))/ix;
