...
sub check_hashcash_value {
  my ($self, $valmin, $valmax) = @_;
  my $val = $self->_run_hashcash();
...
sub check_hashcash_double_spend {
  my ($self) = @_;
  $self->_run_hashcash();
  return ($self->{hashcash_double_spent});
}

sub _run_hashcash {
  my ($self) = @_;

  if (defined $self->{hashcash_value}) { return $self->{hashcash_value}; }

  $self->{hashcash_value} = 0;
  my $hc = $self->get ("X-Hashcash");
  ...
  my $accept = $self->{conf}->{hashcash_accept};
  if (!$self->_check_hashcash_resource ($accept, $rsrc)) {
  ...
  if ($self->was_hashcash_token_double_spent ($hc)) {
    $self->{hashcash_double_spent} = 1;
    return 0;
  }

  $self->{hashcash_value} = $value;
  return $value;
}
...
sub _check_hashcash_resource {
  ...
  foreach my $regexp (values %{$list})
  {
    ...
    $regexp =~ s/\%u/$self->{main}->{username}/gs;
