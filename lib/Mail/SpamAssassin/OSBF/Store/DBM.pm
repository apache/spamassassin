...
package Mail::SpamAssassin::OSBF::Store::DBM;
...
use Mail::SpamAssassin::OSBF::Store;
...
@ISA = qw( Mail::SpamAssassin::OSBF::Store );
...
sub tie_db_readonly {
  ...
  if (!$self->HAS_DBM_MODULE) {
    dbg("osbf: " . $self->DBM_MODULE . " module not installed, cannot use OSBF");
    return 0;
  }
  ...
  my $main = $self->{osbf}->{main};
  if (!defined($main->{conf}->{osbf_path})) {
    dbg("osbf: osbf_path not defined");
    return 0;
  }

  $self->read_db_configs();

  my $path = $main->sed_path($main->{conf}->{osbf_path});
  ...
  if (!$found) {
    dbg("osbf: no dbs present, cannot tie DB R/O: ${path}_toks");
    return 0;
  }
  ...
  dbg("osbf: found bayes db version ".$self->{db_version});

  # If the DB version is one we don't understand, abort!
  if ($self->_check_db_version() != 0) {
    warn("osbf: bayes db version ".$self->{db_version}." is not able to be used, aborting!");
    $self->untie_db();
    return 0;
  }
...
failed_to_tie:
  warn "osbf: cannot open bayes databases ${path}_* R/O: tie failed: $!\n";
  foreach my $dbname (@DBNAMES) {
    my $db_var = 'db_'.$dbname;
    next unless exists $self->{$db_var};
    dbg("osbf: untie-ing DB file $dbname");
    untie %{$self->{$db_var}};
  }
...
sub tie_db_writable {
  ...
  if (!$self->HAS_DBM_MODULE) {
    dbg("osbf: " . $self->DBM_MODULE . " module not installed, cannot use bayes");
    return 0;
  }
  ...
  if ($self->{already_tied} && $self->{is_locked} == 1) {
    ...
    # don't refresh too frequently; once every 15 secs should be plenty
    my $now = time;
    if (!$self->{last_refresh} || ($now - $self->{last_refresh} > 15)) {
  ...
  if (!-d $parentdir) {
    ...
    eval {
      mkpath($parentdir, 0, (oct($main->{conf}->{osbf_file_mode}) & 0777));
    };
  }
  ...
  if ($main->{locker}->safe_lock($path, $tout, $main->{conf}->{osbf_file_mode}))
  {
    $self->{locked_file} = $path;
    $self->{is_locked} = 1;
  } else {
    warn "osbf: cannot open bayes databases ${path}_* R/W: lock failed: $!\n";
    return 0;
  }
  ...
  foreach my $dbname (@DBNAMES) {
    my $name = $path.'_'.$dbname;
    my $db_var = 'db_'.$dbname;
    dbg("osbf: tie-ing to DB file R/W $name");
  ...
  dbg("osbf: found bayes db version ".$self->{db_version}) if ($found);
  ...
  if ($found && !$self->_upgrade_db()) {
    ...
    dbg("osbf: new db, set db version ".$self->{db_version}." and 0 tokens");
  }
...
failed_to_tie:
  ...
  if ($self->{is_locked}) {
    $self->{osbf}->{main}->{locker}->safe_unlock($self->{locked_file});
    $self->{is_locked} = 0;
  }
  warn "osbf: cannot open bayes databases ${path}_* R/W: tie failed: $err\n";
  return 0;
}
...
sub _upgrade_db {
  ...
  if ($verschk == 1) {
    warn("osbf: bayes db version ".$self->{db_version}." is newer than we understand, aborting!");
    return 0;
  }
  ...
  dbg("osbf: detected bayes db format ".$self->{db_version}.", upgrading");
  ...
  if (-f $jpath) {
    dbg("osbf: old journal file found, removing");
    warn "osbf: couldn't remove $jpath: $!" if (!unlink $jpath);
  }

  if ($self->{db_version} < 2) {
    dbg("osbf: upgrading database format from v".$self->{db_version}." to v2");
    ...
    if (-f $msgc) {
      dbg("osbf: old msgcount file found, removing");
      if (!unlink $msgc) {
        warn "osbf: couldn't remove $msgc: $!";
      }
    }
    ...
    for my $ext ($self->DB_EXTENSIONS) {
      ...
      next unless (-f $newf);
      if (!rename ($newf, $oldf)) {
        warn "osbf: rename $newf to $oldf failed: $!\n";
        return 0;
      }
    }
    ...
    dbg("osbf: upgraded database format from v".$self->{db_version}." to v2 in ".(time - $started)." seconds");
  ...
  if ($self->{db_version} == 2) {
    dbg("osbf: upgrading database format from v".$self->{db_version}." to v3");
    $self->set_running_expire_tok();
    ...
    dbg("osbf: upgraded database format from v".$self->{db_version}." to v3 in ".(time - $started)." seconds");
...
sub untie_db {
  ...
  if ($self->{is_locked}) {
    dbg("osbf: files locked, now unlocking lock");
    $self->{osbf}->{main}->{locker}->safe_unlock ($self->{locked_file});
    $self->{is_locked} = 0;
  }
...
sub token_expiration {
  ...
  if ($kept < 100000) {
    dbg("osbf: token expiration would expire too many tokens, aborting");
...
sub sync_due {
  ...
  my $conf = $self->{osbf}->{main}->{conf};
  return 0 if ($conf->{bayes_journal_max_size} == 0);

  my @vars = $self->get_storage_variables();
  dbg("osbf: DB journal sync: last sync: ".$vars[7],'bayes','-1');
...
sub dump_db_toks {
  ...
  while (my ($tok, $tokvalue) = each %{$self->{db_toks}}) {
    ...
    my $prob = $self->{osbf}->_compute_prob_for_token($tok, $vars[1], $vars[2], $ts, $th);
    $prob ||= 0.5;
...
sub set_running_expire_tok {
  ...
  $self->{osbf}->{main}->{locker}->refresh_lock ($self->{locked_file});
...
sub cleanup {
  ...
  my $umask = umask(0777 - (oct ($conf->{osbf_file_mode}) & 0666));

  if (!open (OUT, ">>".$path)) {
    warn "osbf: cannot write to $path, bayes db update ignored: $!\n";
  ...
  do {
	...
	$err = "  ($!)";
      ...
      warn "osbf: write failed to OSBF journal $path ($len of $nbytes)!$err\n";
      last;
    }

    # This shouldn't happen, but could if the fs is full...
    if ($len != $nbytes) {
      warn "osbf: partial write to bayes journal $path ($len of $nbytes), recovering\n";
      ...
      if (!truncate(OUT, $original_point) || ($write_failure++ > 4)) {
        warn "osbf: cannot write to bayes journal $path, aborting!\n";
	last;
      }
...
sub _sync_journal {
  ...
  if (!$self->{osbf}->{main}->{learn_caller_will_untie}) {
    $self->untie_db();
  }
...
sub _sync_journal_trapped {
  ...
  if (!-r $path) { # will we be able to read the file?
    warn "osbf: bad permissions on journal, can't read: $path\n";
    return 0;
  }
  ...
  # it to be interrupted.
  {
    ...
    if (!rename ($path, $retirepath)) {
      warn "osbf: failed rename $path to $retirepath\n";
      return 0;
    }

    # now read the retired journal
    if (!open (JOURNAL, "<$retirepath")) {
      warn "osbf: cannot open read $retirepath\n";
      return 0;
    }
	...
	warn "osbf: gibberish entry found in journal: $_";
      }
    }
    close JOURNAL;
    ...
    unlink ($retirepath) || warn "osbf: can't unlink $retirepath: $!\n";

    $self->{db_toks}->{$LAST_JOURNAL_SYNC_MAGIC_TOKEN} = $started;

    my $done = time();
    my $msg = ("osbf: synced databases from journal in " .
	       ($done - $started) .
...
sub _get_journal_filename {
  ...
  my $main = $self->{osbf}->{main};
  return $main->sed_path($main->{conf}->{osbf_path}."_journal");
}
...
sub perform_upgrade {
  ...
  eval {
    ...
    opendir(DIR, $dir) || die "osbf: can't opendir $dir: $!";
    my @files = grep { /^osbf_(?:seen|toks)(?:\.\w+)?$/ } readdir(DIR);
    closedir(DIR);
    if (@files < 2 || !grep(/osbf_seen/,@files) || !grep(/osbf_toks/,@files))
    {
      die "osbf: unable to find osbf_toks and osbf_seen, stopping\n";
    }
    ...
    for (@files) {
      my $src = "$dir/$_";
      my $dst = "$dir/old_$_";
      copy($src, $dst) || die "osbf: can't copy $src to $dst: $!\n";
    }
    ...
    # import
    if ($self->tie_db_writable()) {
      $ret += $self->upgrade_old_dbm_files_trapped("$dir/old_osbf_seen",
						   $self->{db_seen});
      $ret += $self->upgrade_old_dbm_files_trapped("$dir/old_osbf_toks",
						   $self->{db_toks});
    }
  ...
  if (defined $eval_stat) {
    warn "osbf: perform_upgrade: $eval_stat\n";
    return 0;
  }
  $ret;
}
...
sub clear_database {
  ...
  dbg("osbf: untie-ing in preparation for removal.");
  ...
  my $path = $self->{osbf}->{main}->sed_path($self->{osbf}->{main}->{conf}->{osbf_path});

  foreach my $dbname (@DBNAMES, 'journal') {
    foreach my $ext ($self->DB_EXTENSIONS) {
      my $name = $path.'_'.$dbname.$ext;
      my $ret = unlink $name;
      dbg("osbf: clear_database: " . ($ret ? 'removed' : 'tried to remove') . " $name");
    }
  }
...
sub restore_database {
  ...
  if (!open(DUMPFILE, '<', $filename)) {
    dbg("osbf: unable to open backup file $filename: $!");
    return 0;
  }
   
  if (!$self->tie_db_writable()) {
    dbg("osbf: failed to tie db writable");
    return 0;
  }
  ...
  unless (tie %new_toks, $self->DBM_MODULE, $tmptoksdbname, O_RDWR|O_CREAT|O_EXCL,
	  (oct ($main->{conf}->{osbf_file_mode}) & 0666)) {
    dbg("osbf: failed to tie temp toks db: $!");
    $self->untie_db();
    umask $umask;
    return 0;
  }
  unless (tie %new_seen, $self->DBM_MODULE, $tmpseendbname, O_RDWR|O_CREAT|O_EXCL,
	  (oct ($main->{conf}->{osbf_file_mode}) & 0666)) {
    dbg("osbf: failed to tie temp seen db: $!");
    untie %new_toks;
  ...
  if ($line =~ m/^v\s+(\d+)\s+db_version/) {
    ...
    dbg("osbf: database version must be the first line in the backup file, correct and re-run");
    untie %new_toks;
    untie %new_seen;
    $self->_unlink_file($tmptoksdbname);
  ...
  unless ($db_version == 2 || $db_version == 3) {
    warn("osbf: database version $db_version is unsupported, must be version 2 or 3");
    untie %new_toks;
    untie %new_seen;
    $self->_unlink_file($tmptoksdbname);
    $self->_unlink_file($tmpseendbname);
  ...
  while (my $line = <DUMPFILE>) {
	...
	dbg("osbf: restore_database: skipping unknown line: $line");
      }
    }
	...
	dbg("osbf: token has zero spam and ham count, skipping");
	next;
      }
	...
	$token_warn_p = 1;
      ...
      if ($token_warn_p) {
	dbg("osbf: token ($token) has the following warnings:\n".join("\n",@warnings));
      }
	...
	dbg("osbf: unknown seen flag ($flag) for line: $line, skipping");
	next;
      }

      unless ($msgid) {
	dbg("osbf: blank msgid for line: $line, skipping");
	next;
      }
      ...
      dbg("osbf: skipping unknown line: $line");
      next;
    }
  }
  close(DUMPFILE);
  ...
  unless (defined($num_spam)) {
    dbg("osbf: unable to find num spam, please check file");
    $error_p = 1;
  }

  unless (defined($num_ham)) {
    dbg("osbf: unable to find num ham, please check file");
    $error_p = 1;
  }

  if ($error_p) {
    dbg("osbf: error(s) while attempting to load $filename, correct and re-run");
  ...
  unless ($self->_rename_file($tmptoksdbname, $toksdbname)) {
    dbg("osbf: error while renaming $tmptoksdbname to $toksdbname: $!");
    return 0;
  }
  unless ($self->_rename_file($tmpseendbname, $seendbname)) {
    dbg("osbf: error while renaming $tmpseendbname to $seendbname: $!");
    dbg("osbf: database now in inconsistent state");
    return 0;
  }

  dbg("osbf: parsed $line_count lines");
  dbg("osbf: created database with $token_count tokens based on $num_spam spam messages and $num_ham ham messages");
...
sub tok_unpack {
  ...
  # other formats would go here...
    ...
    warn "osbf: unknown packing format for bayes db, please re-learn: $packed";
    return (0, 0, 0);
  }
}
