...
Mail::SpamAssassin::OSBF::Store - OSBFian Storage Module

=head1 DESCRIPTION

This is the public API for the OSBFian store methods.  Any implementation of
the storage module must implement these methods.

=cut

package Mail::SpamAssassin::OSBF::Store;
...
public class (Mail::SpamAssassin::OSBF::Store) new (Mail::SpamAssassin::Plugin::OSBF $bayes)

Description:
This method creates a new instance of the
Mail::SpamAssassin::OSBF::Store object.  You must pass in an instance of the
Mail::SpamAssassin::Plugin::OSBF object, which is stashed for use throughout
the module.
...
object and then calls the Mail::SpamAssassin::Plugin::OSBF read_db_configs method.
...
sub tie_db_readonly {
  my ($self) = @_;
  die "osbf: tie_db_readonly: not implemented\n";
}
...
sub tie_db_writable {
  my ($self) = @_;
  die "osbf: tie_db_writable: not implemented\n";
}
...
sub untie_db {
  my $self = shift;
  die "osbf: untie_db: not implemented\n";
}
...
sub calculate_expire_delta {
  ...
  die "osbf: calculate_expire_delta: not implemented\n";
}
...
sub token_expiration {
  ...
  die "osbf: token_expiration: not implemented\n";
}
...
sub expire_old_tokens {
  ...
  if (defined $eval_stat) {	# if we died, untie the dbs.
    warn "osbf: expire_old_tokens: $eval_stat\n";
    return 0;
  }
  $ret;
}
...
sub expire_old_tokens_trapped {
  ...
  if ( $vars[10] > time ) {
    dbg("osbf: expiry found newest atime in the future, resetting to current time");
    $vars[10] = time;
  }

  # How many tokens do we want to keep?
  my $goal_reduction = int($self->{expiry_max_db_size} * $self->{expiry_pct});
  dbg("osbf: expiry check keep size, ".$self->{expiry_pct}." * max: $goal_reduction");
  # Make sure we keep at least 100000 tokens in the DB
  if ( $goal_reduction < 100000 ) {
    $goal_reduction = 100000;
    dbg("osbf: expiry keep size too small, resetting to 100,000 tokens");
  }
  # Now turn goal_reduction into how many to expire.
  $goal_reduction = $vars[3] - $goal_reduction;
  dbg("osbf: token count: ".$vars[3].", final goal reduction size: $goal_reduction");

  if ( $goal_reduction < 1000 ) { # too few tokens to expire, abort.
    dbg("osbf: reduction goal of $goal_reduction is under 1,000 tokens, skipping expire");
  ...
  dbg("osbf: first pass?  current: ".time().", Last: ".$vars[4].", atime: ".$vars[8].", count: ".$vars[9].", newdelta: $newdelta, ratio: $ratio, period: ".$self->{expiry_period});
  ...
  if ( (time() - $vars[4] > 86400*30) || ($vars[8] < $self->{expiry_period}) || ($vars[9] < 1000)
    ...
    dbg("osbf: can't use estimation method for expiry, unexpected result, calculating optimal atime delta (first pass)");
    ...
    dbg("osbf: expiry max exponent: ".$self->{expiry_max_exponent});
    ...
    if (would_log('dbg', 'bayes')) {
      dbg("osbf: atime\ttoken reduction");
      dbg("osbf: ========\t===============");
      for(my $i = 1; $i<=$max_expire_mult; $i <<= 1) {
	dbg("osbf: ".$start*$i."\t".(exists $delta{$i} ? $delta{$i} : 0));
      }
    }
    ...
    if ( !exists $delta{$max_expire_mult} || $delta{$max_expire_mult} < 1000 ) {
      dbg("osbf: couldn't find a good delta atime, need more token difference, skipping expire");
      $self->set_last_expire(time());
      $self->remove_running_expire_tok(); # this won't be cleaned up, so do it now.
      return 1; # we want to indicate things ran as expected
    }
    ...
    dbg("osbf: first pass decided on $newdelta for atime delta");
  }
  else { # use the estimation method
    dbg("osbf: can do estimation method for expiry, skipping first pass");
  }
...
sub sync_due {
  my ($self) = @_;
  die "osbf: sync_due: not implemented\n";
}
...
sub expiry_due {
  ...
  return 1 if ($self->{osbf}->{main}->{learn_force_expire});

  # if config says not to auto expire then no need to continue
  return 0 if ($self->{osbf}->{main}->{conf}->{bayes_auto_expire} == 0);
  ...
  if (!$self->{osbf}->{main}->{ignore_safety_expire_timeout}) {
  ...
  dbg("osbf: DB expiry: tokens in DB: $ntoks, Expiry max size: ".$self->{expiry_max_db_size}.", Oldest atime: ".$vars[5].", Newest atime: ".$vars[10].", Last expire: ".$vars[4].", Current time: ".time(),'bayes','-1');
...
C<$msgid> is not found.  Plus additional values (TODO)

=cut

sub seen_get {
  my ($self, $msgid) = @_;
  die "osbf: seen_get: not implemented\n";
}
...
one of two values 's' for spam and 'h' for ham, plus additional
values (TODO).
...
sub seen_put {
  ...
  die "osbf: seen_put: not implemented\n";
}
...
sub seen_delete {
  my ($self, $msgid) = @_;
  die "osbf: seen_delete: not implemented\n";
}
...
the OSBF storage implementation.
...
sub get_storage_variables {
  my ($self) = @_;
  die "osbf: get_storage_variables: not implemented\n";
}
...
sub dump_db_toks {
  ...
  die "osbf: dump_db_toks: not implemented\n";
}
...
sub set_last_expire {
  my ($self, $time) = @_;
  die "osbf: set_last_expire: not implemented\n";
}
...
sub get_running_expire_tok {
  my ($self) = @_;
  die "osbf: get_running_expire_tok: not implemented\n";
}
...
sub set_running_expire_tok {
  my ($self) = @_;
  die "osbf: set_running_expire_tok: not implemented\n";
}
...
sub remove_running_expire_tok {
  my ($self) = @_;
  die "osbf: remove_running_expire_tok: not implemented\n";
}
...
sub tok_get {
  my ($self, $token) = @_;
  die "osbf: tok_get: not implemented\n";
}
...
sub tok_get_all {
  my ($self, $tokens) = @_;
  die "osbf: tok_get_all: not implemented\n";
}

=item multi_tok_value_change

public instance (Boolean) multi_tok_value_change (\% $weights,
					          String $atime)

Description:
This method takes a C<$weights> hash ref and writes the associated
[ spam, ham ] value pairs to the store, along with updating each tokens
atime with C<$atime>.
...
sub multi_tok_value_change {
  ...
  die "osbf: multi_tok_value_change: not implemented\n";
}
...
sub nspam_nham_get {
  my ($self) = @_;
  die "osbf: nspam_nham_get: not implemented\n";
}
...
sub nspam_nham_change {
  ...
  die "osbf: nspam_nham_change: not implemented\n";
}
...
sub tok_touch {
  ...
  die "osbf: tok_touch: not implemanted\n";
}
...
sub tok_touch_all {
  ...
  die "osbf: tok_touch_all: not implemanted\n";
}
...
sub cleanup {
  my ($self) = @_;
  die "osbf: cleanup: not implemented\n";
}
...
sub get_magic_re {
  my ($self) = @_;
  die "osbf: get_magic_re: not implemented\n";
}
...
sub perform_upgrade {
  my ($self, $opts) = @_;
  die "osbf: perform_upgrade: not implemented\n";
}
...
sub clear_database {
  my ($self) = @_;
  die "osbf: clear_database: not implemented\n";
}
...
sub backup_database {
  my ($self) = @_;
  die "osbf: backup_database: not implemented\n";
}
...
sub restore_database {
  ...
  die "osbf: restore_database: not implemented\n";
}
...
Description:
This method returns whether or not the OSBF DB is available in a
readable state.
...
sub db_readable {
  my ($self) = @_;
  die "osbf: db_readable: not implemented\n";
}
...
sub db_writable {
  my ($self) = @_;
  die "osbf: db_writable: not implemented\n";
}
