...
sub make_conf {
  ...
  Mail::SpamAssassin::Plugin::Redirectors::_add_redirector_entry($conf, 'bare.example', 'head');
  Mail::SpamAssassin::Plugin::Redirectors::_add_redirector_entry($conf, '.dotted.example', 'head');
  Mail::SpamAssassin::Plugin::Redirectors::_add_redirector_entry($conf, '.r.af.d.multi.example/tr/cl/', 'get');
...
my @cases = (
  ...
  [ 'https://bare.example/',          'head', 'bare domain matches exact host' ],
  [ 'https://www.bare.example/x',     'head', 'bare domain matches www subdomain' ],
  [ 'https://mail.bare.example/x',    undef,  'bare domain does NOT match other subdomains' ],

  # Leading-dot entry: subdomains only, NOT the bare domain itself.
  [ 'https://dotted.example/x',     undef,  'leading-dot does NOT match bare domain' ],
  [ 'https://www.dotted.example/x', 'head', 'leading-dot matches www subdomain' ],
  [ 'https://a.b.dotted.example/x', 'head', 'leading-dot matches deep subdomain' ],

  # Multi-label leading-dot: still excludes the bare-domain form.
  [ 'https://r.af.d.multi.example/tr/cl/abc',         undef, '.r.af.d.multi.example does NOT match its own bare form' ],
  [ 'https://x.r.af.d.multi.example/tr/cl/abc',       'get', '.r.af.d.multi.example matches subdomain' ],
  [ 'https://gfbgghj.r.af.d.multi.example/tr/cl/abc', 'get', '.r.af.d.multi.example matches deeper subdomain' ],

  # Path-prefix gating, trailing slash in config (/tr/op/).
  [ 'https://gfbgghj.r.af.d.multi.example/tr/op/abc', undef, 'path /tr/op blocked when only /tr/cl/ configured' ],
...
# original false positives on a newsletter site, not just url/u.
my $embedded_conf =
  { url_redirector_params => qr/(?:url|u|uri|r|redir|redirect)=(.*)/i };
my @embedded = (
  # [ uri, expected_extraction (undef = no embedded uri), desc ]
  [ 'https://safe.example/?url=https://evil.example/path', 'https://evil.example/path',
    'explicit scheme is extracted' ],
  [ 'https://safe.example/?url=evil.example/landing', 'http://evil.example/landing',
    'bare host.tld/path is extracted' ],
  [ 'https://safe.example/?url=//evil.example/path', 'http://evil.example/path',
    'scheme-relative //host is extracted' ],
  [ 'https://safe.example/?url=//////evil.example/path', 'http://evil.example/path',
    'scheme-relative host with padded slash run is extracted' ],
  [ 'https://login.provider.example//////////////////////////////common/oauth2/v2.0/authorize?state=1202',
    undef,
  ...
  [ 'https://safe.example/?url=https%253A%252F%252Fevil.example%252Fpath',
    'https://evil.example/path',
    'double-encoded scheme is decoded' ],
  [ 'https://safe.example/?url=https://evil.example/path&utm_source=news&id=42',
    'https://evil.example/path',
    'extraction stops at the parameter boundary, not end of querystring' ],
  [ 'https://safe.example/?url=https%3A%2F%2Fevil.example%2Fpath&utm_source=news',
    'https://evil.example/path',
    'encoded target stops at the parameter boundary' ],
  # Modelled on a real sample: a padded slash run in the path plus a
  # double-encoded uri= target and trailing binary padding. The uri= target is
  # the real destination and must come back clean.
  [ 'https://login.provider.example//////////////////////////////common/oauth2/v2.0/authorize?state=1202&scope=openid+profile+https%253A%252F%252Fgraph.provider.example%252FUser.Read&prompt=none&client_id=967ac14a-0b4d-4175-ab8d-694fdba23afa&uri=https%253A%252F%252Fdeveloper.target.example%252Fdashboard%252Fsession%252Fuser%252Fverify%252Fstep1&%255Ca3edq%250C+2e4c%250D%250A%2593bb66f835%2509%258C2979X%25BCint+Builder.Decode%250A%2509Context+%253A%253D+FlowEmail+%255B+OffsetStream+%253A=%2520Token%2509Data%2520%257C%2520Email%257Dfor%2520Stream%253A%253DPayloadBuilder+;+ValueContext+%257D+Trace%250A%2509Decode+.+SignalVector+%257B%2520Offset%257D%250Aa78c998ef06b569e%2597%25E9%252A%25CBa93627d06a07eba872664f4a92c74eae25f60308d1cad09a16e7beef0b79c03d8bd7528c4a7efc8bdb3fc053evar%252BVector-Secret%25250A%252509Decode%252B%25253B%252BBuffer%25250A%252509Encode%252B-%252BSession%25250A%252509Decode%252B%25255D%252BPayload%25250A%252509Offset%252B%25252B%252BBuilder%25250A%252509Builder%252B%252528%252BToken%25250A%252509Encode%252B.%252BKey%25250A%252509Context%252B%25257B%252BToken%25250A%25257D%25250Aelse%252BDecode%25252CBuilder%25250A%252509Stream%252B%25253A%25253D%252BHeader%25250A%252509Vector%252B%252526%252BVector%25250A%252509Payload%252B%25253D%252BBuilder%25250A%252509Value%252B%25257C%252BPayload%25250A%252509Secret%252B%25253D%252BBuffer%25250A%25257D%25250Aswitch%252BPayload%25252CSession%25250A%252509Payload%252B%252529%252BToken%25250A%252509Payload%252B%252526%252BBuilder%25250A%252509Data%252B%25257C%252BDecode%25250A%252509Secret%252B%25255B%252BStream%25250A%25257D%25250Astring%252BBody%252529Session%25250A%252509Session%252B%252528%252BTrace%25250A%252509Buffer%252B%25257B%252BSession%25250A%252509Vector%252B%25252A%252BVector%25250A%252509Context%252B%25253B%252BToken%25250A%252509Value%252B%25252A%252BData%25250A%252509Encode%252B%25253B%252BFlow%25250A%252509Trace%252B%252529%252BTrace%25250A%25257D%25250Aint%252BToken%25257CSignal%25250A%252509Header%252B%25255D%252BFlow%25250A%252509Body%252B.%252BKey%25250A%252509Vector%252B%252528%252BSignal%25250A%252509Session%252B%25252C%252BData%25250A%25257D%25250Ac2FuZGVlcEBmdmNvbS5hZQ==',
    'https://developer.target.example/dashboard/session/user/verify/step1',
    'padded oauth lure yields the real embedded target' ],
  [ 'https://safe.example/?url=evil.example%2Fp%3Fx%3D1%26tid%3Dgood.example',
    'http://evil.example/p?x=1&tid=good.example',
  ...
  [ 'https://safe.example/?foo=https://plain.example', 'https://plain.example',
    'unlisted param with a plain embedded uri is extracted' ],
  [ 'https://safe.example/?foo=https%3A%2F%2Fplain.example', 'https://plain.example',
    'unlisted param with an encoded embedded uri is extracted' ],
  [ 'https://safe.example/?foo=https%253A%252F%252Fplain.example', 'https://plain.example',
  ...
  [ 'https://safe.example/?foo=https%3A%2F%2Fevil.example%2Fp%3Fx%3D1%26tid%3Dgood.example',
    'https://evil.example/p?x=1&tid=good.example',
    'unlisted param keeps encoded separators inside the embedded value' ],
  [ 'https://safe.example/?foo=https%253A%252F%252Fevil.example%252Fp%253Fa%253D1%2526b%253D2',
    'https://evil.example/p?a=1&b=2',
  ...
  [ 'https://localhost/?url=https://evil.example/path', 'https://evil.example/path',
    'a dotless host is still searched for an embedded uri' ],
  [ 'https://news.example/signup?foo=bar&r=to8ex', undef,
    'bare referral token (r=) is NOT an embedded uri' ],
  [ 'https://open.news.example/notes?utm_campaign=open-in-app&redirect=app-store-no-desktop', undef,
...
# when there is one, and the whole value when there is not.
{
  ...
  my $r = Mail::SpamAssassin::Plugin::Redirectors::_extract_embedded_uri(
    Mail::SpamAssassin::URI->new('https://safe.example/?u=12345|https://evil.example/p'),
    $wrapped_conf);
  is($r, 'https://evil.example/p', 'capture group pulls the target out of a wrapper');
}

{
  ...
  my $r = Mail::SpamAssassin::Plugin::Redirectors::_extract_embedded_uri(
    Mail::SpamAssassin::URI->new('https://safe.example/?u=https://evil.example/p'),
    $nocap_names);
  is($r, 'https://evil.example/p', 'without a capture group the whole value is used');
}
...
# extractor must bail rather than fabricate a bare "http://".
{
  ...
  my $r = Mail::SpamAssassin::Plugin::Redirectors::_extract_embedded_uri(
    Mail::SpamAssassin::URI->new('https://news.example/signup?foo=bar&r=to8ex'),
    $nocap_conf);
...
{
  my $conf = { url_redirector_params => qr/(?:url|u)=(.*)/i };
  Mail::SpamAssassin::Plugin::Redirectors::_add_redirector_entry($conf, '.multi.example/tr/cl/', 'get');
  Mail::SpamAssassin::Plugin::Redirectors::_add_redirector_entry($conf, '.multi.example/tr/click/', 'get');

  Mail::SpamAssassin::Plugin::Redirectors::_clear_redirector_entry($conf, '.multi.example/tr/cl/');
  ok(!Mail::SpamAssassin::Plugin::Redirectors::_lookup_redirector(
       Mail::SpamAssassin::URI->new('https://x.multi.example/tr/cl/a'), $conf),
     'clear removes /tr/cl/ path');
  ok( Mail::SpamAssassin::Plugin::Redirectors::_lookup_redirector(
       Mail::SpamAssassin::URI->new('https://x.multi.example/tr/click/a'), $conf),
     'clear leaves /tr/click/ path');

  Mail::SpamAssassin::Plugin::Redirectors::_clear_redirector_entry($conf, '.multi.example/tr/click/');
  ok(!Mail::SpamAssassin::Plugin::Redirectors::_lookup_redirector(
       Mail::SpamAssassin::URI->new('https://x.multi.example/tr/click/a'), $conf),
     'clear removes last path');
  ok(!exists $conf->{url_redirector_suffix}->{'multi.example'},
